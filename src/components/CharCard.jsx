...
function CharCard({character}){
  ...
  const normalAptitudes = character.normal.aptitudes.map((aptitude)=><Aptitude
    name={aptitude.name}
    text={aptitude.text}/>);
  
  const exaltedAptitudes = character.exalted.aptitudes.map((aptitude)=><Aptitude
    name={aptitude.name}
    text={aptitude.text}/>);

  const normalAttacks= character.normal.attacks.map((attack)=><Attack
    name={attack.name}
    type={attack.type}
    dice={attack.dice}
    dmg={attack.dmg}
  ...
  const exaltedAttacks= character.exalted.attacks.map((attack)=><Attack
    name={attack.name}
    type={attack.type}
    dice={attack.dice}
    dmg={attack.dmg}
  ...
  return (
    ...
    <Card>
      ...
      <img alt="Character portrait" style={{ width: "200px", height: "200px", borderRadius : "50%" }}src={Picture} />
      ...
      <Col>
        <Title name={character.charName} keywords={character.charKeywords}/>
        <Card.Text>
                <Row>
          <BoxDice/><BoxDice/><BoxDice/><BoxDice/>
        </Row>
        <Row>
        ...
        <UniqueAptitudes>
          ...
          {!exalted && <Sentier name={character.gloryRule.name} text={character.gloryRule.text}/>}
        </Card.Text>
        </Col>
